...
chosen = utils.chosen_epochs(path=file_path, write_=False)
x = 1
...
if tasks["acc_loss"]:
    for measure in ["Accuracy", "Loss"]:
        for type_ in ["Train", "Validation"]:
            ...
            for i in range(5):
                try:
                    plt.plot(range(1, epochs + 1), type_df[type_df.Split == i][measure], label=f"fold {i}",
                             color=colors[i])
...
if tasks["conf_mat"]:
    ...
    for vid in list_of_videos:
        ...
        recog_content.extend([float(actions_dict[label]) for label in
                              read_file(paths.results_dir + "/" + vid.split('.')[0]).split('\n')[1].split()])
