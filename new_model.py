...
class MS_TCN2(nn.Module):
    def __init__(self, num_layers_PG, num_layers_R, num_R, num_f_maps, dim, num_classes):
        ...
        self.PG_kin = Prediction_Generation(num_layers_PG, num_f_maps, utils.kin_features_dim, num_classes)

        self.Rs = nn.ModuleList(
            [copy.deepcopy(Refinement(num_layers_R, num_f_maps, num_classes, num_classes)) for s in range(num_R)])

    def forward(self, batch_input, batch_kin):
        out = self.PG(batch_input)
        outputs = out.unsqueeze(0)
        for R in self.Rs:
            out = R(F.softmax(out, dim=1))
            outputs = torch.cat((outputs, out.unsqueeze(0)), dim=0)

        out_kin = self.PG_kin(batch_kin)
        outputs_kin = out_kin.unsqueeze(0)
        for R in self.Rs:
...
class Trainer:
    ...
    def predict(self, model_dir, results_dir, features_path, vid_list_file, epoch, actions_dict, device, sample_rate,
        ...
        with torch.no_grad():
            self.model.to(device)
            # self.model.load_state_dict(torch.load(model_dir + f"-new/split-{split}-epoch-" + str(epoch) + ".model"))
            ...
            for vid in tqdm(list_of_vids):
                ...
                kinematics = kinematics[:, ::sample_rate]

                input_x = torch.tensor(features, dtype=torch.float)
                input_x.unsqueeze_(0)
                input_x = input_x.to(device)

                input_kin = torch.tensor(kinematics, dtype=torch.float)
                input_kin.unsqueeze_(0)
                input_kin = input_kin.to(device)

                predictions = self.model(input_x, input_kin)
                _, predicted = torch.max(predictions[-1].data, 1)
